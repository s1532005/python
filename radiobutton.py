...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # ラジオボタンのラベルをリスト化する
        rdo_txt = ['Hilight','Funny']
        # ラジオボタンの状態
        rdo_var = tk.IntVar()

        # ラジオボタンを動的に作成して配置
        for i in range(len(rdo_txt)):
            rdo = tk.Radiobutton(tki, value=i, variable=rdo_var, text=rdo_txt[i]) 
            rdo.place(x=50, y=30 + (i * 24))

        # ボタン作成 
        btn = tk.Button(tki, text='ラジオボタン取得', command=btn_click)
    ...
    def btn_click():
        num = rdo_var.get()
        tk.messagebox.showinfo('チェックされた項目', rdo_txt[num])
